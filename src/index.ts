...
export default class MqttRequest {
  ...
  public constructor(mqttClient: any) {
    ...
    this._mqtt.on('message', (topic:string, message:any) => {this._handleMessage(topic, message)});
    this._cleanTimer = setInterval(this.__cleanQueue.bind(this), MqttRequest.timeout * 5);
  }

  public request(topic:string, callback: (message: any) => void, payload?:any,) {
    const uuid = nanoid();
  ...
  private _handleMessage(topic:string, payload:any):void{
    if(topic.search("@request") >= 0) {this.__handleRequest(topic, payload);}
    if(topic.search("@response") >= 0) {this.__handleResponse(topic, payload);}
  }
  ...
  private __cleanQueue() {
    const now = Date.now();
    this._queue = this._queue.reduce((t, it) => it.expires < now ? [...t, it] : t, <MqttRequestQueueItem[]>[]);
  }
}
